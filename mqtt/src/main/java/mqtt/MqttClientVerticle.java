		...
		mqttClient.connect(1883, "localhost", s -> {
			...
			mqttClient.publishHandler(handler -> {
				...
				if (sensorData.getHumedad() < 50 || sensorData.getTemperatura() > 24 ) {
					Rele estadoActuador = getReleIdGroup(miGestorRest, sensorData.getIdGroup());
...
//							false);
	...
	private static Rele getReleIdGroup(ApiService gestor, Integer idGroup) {
		Rele res = null;
		try {
			res = gestor.getActuadorByIdGroup(idGroup);
		} catch (Exception e) {
	...
	private static DHT11 getDHT11IdGroup(ApiService gestor, Integer idGroup) {
		DHT11 res = null;
		try {
			res = gestor.getSensorByIdGroup(idGroup);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
